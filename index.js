...
class Inject extends Colors {
  ...
  async run(M, callable) {
    ...
    const S = new States();
    await Shell.write(
      `${this.CYAN}Injecting ${this.PINK}${this.outputType}${this.CYAN} into ${this.PINK}Base IPAs${this.CYAN}. If ${this.hasClean ? "a " : ""}${this.PINK}${this.type}${this.CYAN} has been ${this.GREEN}successfully${this.CYAN} injected in all IPAs, it will look like this: ${this.BLUE}"${S.SUCCESS} ${this.hasClean ? `Example ` : ""}${this.type}${this.BLUE}"\n`,
    );
    ...
    case 0: {
      ...
      await M.Main(async () => {
        ...
        await Shell.runSilently(
          `zip -q -r Dist/Rosiecord-${ipaName.split("_")[1]}_GGSans-Font.ipa Payload & wait $!`,
          async (stderr, _) => {
        ...
        for (const Font of ipaList.filter((ipa) => ipa !== "GGSans")) {
          await Shell.runSilently(
            `unzip -qq -o Dist/Rosiecord-${ipaName.split("_")[1]}_GGSans-Font.ipa`,
          );
          await Shell.runSilently(
            `cp -rf Fonts/ttf/${Font}/* Payload/Discord.app/`,
          );
          await Shell.runSilently(
            `zip -q -r Dist/Rosiecord-${ipaName.split("_")[1]}_${Font}-Font.ipa Payload & wait $!`,
          );
          await Shell.runSilently(`rm -rf Payload & wait $!`);
...
const main = async () => {
  ...
  await Shell.write(
    `${S.SUCCESS} Successfully built ${M.PINK}Rosiecord${M.GREEN} in ${M.CYAN}${(END_TIME - START_TIME) / 1000} seconds${M.GREEN}.`,
  );
};
await main();
